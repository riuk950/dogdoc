...
- **Objetivo:** Definir las estructuras que configuran y encapsulan toda la información necesaria para el informe clínico.
- **Alcance:**
  - `lib/domain/model/report_configuration.dart` (`petId`, `startDate`, `endDate`, `includePhotos`, `customNotes`).
...
- **Dependencias:** Ninguna.
- **Criterios resueltos:** CA-01, CA-02, CA-03, CA-04.
...
- **Alcance:**
  ...
  - `lib/domain/usecases/export/prepare_report_data_usecase.dart` (consulta a Drift de mascotas, logs en rango, KPIs y patrones clínicos).
...
- **Criterios resueltos:** CA-01, CA-04, CA-07.
...
- **Alcance:**
  ...
  - Anexo Fotográfico: Renderizado en cuadrícula de fotos de lesiones dermatológicas con fecha y zona anatómica.
  - Pie de página legal con disclaimer veterinario y paginación.
- **Dependencias:** TASK-01, TASK-03.
- **Criterios resueltos:** CA-01, CA-02, CA-03, CA-08, CA-09.
- **Método de validación:** Test unitario comprobando que el archivo generado empieza con `%PDF-` y que el recuento de páginas se reduce al desactivar `includePhotos`.
...
- **Objetivo:** Aislar la compilación del PDF y el procesamiento de fotos en un hilo secundario para garantizar fluidez de la UI.
- **Alcance:**
...
- **Criterios resueltos:** CA-02, CA-07.
- **Método de validación:** Test unitario verificando la compresión de bytes y el manejo de excepciones por archivos faltantes.
