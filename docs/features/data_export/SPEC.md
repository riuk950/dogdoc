...
4. Al pulsar *"Generar Informe"*:
   ...
   - El caso de uso compila los registros de Drift SQLite, analiza los KPIs y prepara las imágenes locales optimizadas en un `Isolate` de segundo plano.
...
1. En `ReportPreviewScreen`, el usuario visualiza el documento maquetado:
   ...
   - **Anexo Dermatológico (si está activo):** Cuadrícula de fotos de lesiones fechadas con indicación de zona anatómica (máximo 4 por página, optimizadas a $< 200$ KB por foto).
...
1. **Rendimiento y Memoria (Ejecución en Background / Isolate):**
   - El renderizado del PDF y el procesamiento de imágenes de lesiones se realiza en un `Isolate` secundario (`compute()`), evitando cualquier bloqueo del hilo principal de la UI o congelamiento de animaciones durante la generación de reportes extensos.
...
- **CA-01: Generación exitosa de documento PDF con datos del paciente.**
...
- **CA-02: Inclusión y compresión del anexo fotográfico de lesiones.**
...
- **CA-03: Exclusión de fotos cuando el interruptor está inactivo.**
...
- **CA-04: Filtro por rangos de fechas (7d, 30d, 3m, personalizado).**
...
- **CA-05: Visualización de pantalla de previsualización interactiva (`PdfPreview`).**
...
- **CA-06: Despliegue de la hoja de compartir nativa (Share Sheet).**
...
- **CA-08: Inclusión de la gráfica de prurito en la portada del PDF.**
...
- **CA-09: Inclusión de disclaimer veterinario en el documento.**
...
2. **Galería de Fotos de Lesiones:** Interruptor configurable (activado por defecto) que permite incluir o excluir las fotos de lesiones, aplicando compresión automática a $< 200$ KB por foto para mantener el PDF ligero ($< 3\text{--}5$ MB).
3. **Previsualización Interactiva:** Pantalla de vista previa previa (`PdfPreview`) que permite revisar las páginas antes de compartir o imprimir.
4. **Rango de Fechas:** Selector flexible con presets rápidos (7 días, 30 días, 3 meses) y selector de rango personalizado.
5. **Gráfica Evolutiva en el PDF:** Se renderiza la curva de picor en la portada del PDF para ofrecer una lectura visual inmediata de la respuesta al tratamiento.
